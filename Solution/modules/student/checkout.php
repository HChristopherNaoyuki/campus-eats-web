 ...
 *
 * Handles the checkout process for student and standard users.
 * Includes service fee calculation, tax calculation, rounding rules,
 * and student discount (2.5%) for Student role users only.
 *
 * CORRECTIONS (Version 34.0):
 * - Service fee boundary at exactly R1000 now returns no fee
 *   per process document Section 10.1
 * - Payment method validation now uses PAYMENT_METHOD_* constants
 *   instead of hardcoded literal strings
 * - Uses the shared escapeOutput() helper
 * - Uses the shared order number generator with 8-character suffix
 *
 * SOURCE: Issue report - items 10, 11, 23, 26
 * SOURCE: campus-eats-process-document.pdf Section 10.1
 *
 * @version 34.0
 */
 ...
 *
 * Rule from the process document Section 10.1:
 * - Under R500: 10 percent fee
 * - From R500 up to but not including R1000: 6.5 percent fee
 * - R1000 and above: no fee
 *
 * CORRECTION: The boundary is strictly less than R1000 for the mid
 * tier. A subtotal of exactly R1000 receives no fee.
 *
 * @param float $subtotal The order subtotal
 * @return float The service fee
 */
function calculateServiceFee($subtotal)
{
    ...
    elseif ($subtotal < SERVICE_FEE_THRESHOLD_HIGH)
    {
...
function calculateStudentDiscount($amount, $isStudent)
{
    if ($isStudent === true)
    {
        return round($amount * STUDENT_DISCOUNT_RATE, 2);
    }
    return 0.0;
}

/**
 * Generates a unique order number in the documented format.
...
/**
 * Returns a list of allowed columns for the requested table.
...
/**
 * Calculates cart totals in the order specified by the process document:
 * service fee, then tax, then rounding, then student discount.
 *
 * @param array $cart The cart items
 * @param bool $isStudent Whether the user is a student
...
{
    $feeMessage = 'No service fee for orders of R1000 or above';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order']))
{
    ...
    {
        $pickupTime = trim(isset($_POST['pickup_time']) ? $_POST['pickup_time'] : '');
        $paymentMethod = trim(isset($_POST['payment_method']) ? $_POST['payment_method'] : '');
        $specialRequests = trim(isset($_POST['special_requests']) ? $_POST['special_requests'] : '');
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // CORRECTION: Payment method validation uses constants.
                    $allowedPaymentMethods = unserialize(ALLOWED_PAYMENT_METHODS_ARRAY);
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    $orderFields['transaction_id'] = 'TD' . gmdate('YmdHis');
                                }
                                    ...
                                    if ($hasTransactionRef)
                                    {
                                        $db->insert(
                                            ...
                                            array(
                                                ...
                                                'amount' => $finalTotal,
                                                'transaction_reference' => 'PAY-' . date('Ymd') . '-' . bin2hex(random_bytes(6))
                                            )
                                        );
                                    }
                                    else
...
if ($orderPlaced)
{
    ...
    $selectFields[] = in_array('subtotal', $orderColumns) ? 'o.subtotal' : '0 as subtotal';
    $selectFields[] = in_array('service_fee', $orderColumns) ? 'o.service_fee' : '0 as service_fee';
    $selectFields[] = ($isStudent && in_array('student_discount', $orderColumns))
        ? 'o.student_discount'
        : '0 as student_discount';
    $selectFields[] = in_array('tax', $orderColumns) ? 'o.tax' : '0 as tax';
    $selectFields[] = in_array('rounding_adjustment', $orderColumns)
                    ...
                    <?php if ($orderPlaced && $orderReceipt): ?>
                        <div class="receipt-container">
                            <div class="receipt-header">
                                ...
                                <p>Thank you for your order, <?php echo escapeOutput($currentUser['full_name']); ?></p>
                            </div>
                            <div class="receipt-body">
                                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4); margin-bottom: var(--space-4);">
                                    <div>
                                        <span style="font-size: 0.75rem; color: var(--gray-500);">Order Number</span>
                                        <p style="font-weight: 600;"><?php echo escapeOutput($orderReceipt['order_number']); ?></p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p style="font-weight: 600;"><?php echo escapeOutput($orderReceipt['vendor_name']); ?></p>
                                    </div>
                                    <div>
                                        <span style="font-size: 0.75rem; color: var(--gray-500);">Pickup Time</span>
                                        <p style="font-weight: 600;"><?php echo escapeOutput($orderReceipt['pickup_time']); ?></p>
                                    </div>
                                    <div>
                                        ...
                                        <div>
                                            ...
                                            <p style="font-weight: 600;">
                                                <span class="discount-badge">
                                                    <i class="fas fa-graduation-cap"></i> Student Discount (2.5%)
                                                </span>
                                            </p>
                                        </div>
                                    <?php endif; ?>
                                ...
                                <?php foreach ($orderItems as $item): ?>
                                    <div class="receipt-item-row">
                                        <span><?php echo $item['quantity']; ?>x <?php echo escapeOutput($item['item_name']); ?></span>
                                ...
                                <div style="margin-top: var(--space-4); padding-top: var(--space-4); border-top: 1px solid var(--gray-200);">
                                    <div class="summary-row">
                                        <span>Subtotal:</span>
                                        <span>R <?php echo number_format(isset($orderReceipt['subtotal']) ? $orderReceipt['subtotal'] : 0, 2); ?></span>
                                    </div>
                                ...
                                <?php if (!empty($orderReceipt['special_requests'])): ?>
                                    <div style="margin-top: var(--space-4); padding: var(--space-3); background: var(--gray-50); border-radius: var(--radius-md);">
                                        ...
                                        <p style="margin-top: var(--space-2);"><?php echo nl2br(escapeOutput($orderReceipt['special_requests'])); ?></p>
                                    </div>
                                <?php endif; ?>
                            </div>
                    ...
                    <?php elseif ($errorMessage): ?>
                        <div class="alert alert-error">
                            ...
                            <div class="alert-content">
                                ...
                                <div class="alert-message"><?php echo escapeOutput($errorMessage); ?></div>
                            </div>
                        </div>
                            ...
                            <div class="order-summary">
                                ...
                                <div class="order-summary-body">
                                    <p style="color: var(--gray-600); margin-bottom: var(--space-4);">
                                        ...
                                        <?php echo escapeOutput($vendorName); ?>
                                    </p>
                                    ...
                                    <?php foreach ($items as $index => $item): ?>
                                        ...
                                        $itemName = isset($item['name'])
                                            ? $item['name']
                                            : (isset($item['item_name']) ? $item['item_name'] : 'Item');
                                        $quantity = isset($item['quantity']) ? (int)$item['quantity'] : 1;
                                        $price = isset($item['price']) ? (float)$item['price'] : 0.0;
                                        $itemSubtotal = $price * $quantity;
                                        ?>
                                        <div class="order-item-row">
                                            <span><?php echo escapeOutput($itemName); ?> x <?php echo $quantity; ?></span>
                                    ...
                                    <div class="summary-breakdown">
                                        ...
                                        <div class="fee-note">
                                            ...
                                            <?php echo escapeOutput($feeMessage); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    <form method="POST" action="" id="checkout-form">
                                        ...
                                        <div class="form-group">
                                            <label class="form-label" for="pickup_time">
                                                Preferred Pickup Time <span class="required">*</span>
                                            </label>
                                            <select id="pickup_time" name="pickup_time" class="form-control" required>
                                                <option value="">Select a time</option>
                                                <?php foreach ($pickupTimes as $time): ?>
                                                    <option value="<?php echo escapeOutput($time['value']); ?>">
                                                        <?php echo escapeOutput($time['label']); ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                            <span class="form-hint">
                                                Pickup available from 10:00 to 16:00 (15-minute intervals)
                                            </span>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="payment_method" name="payment_method" class="form-control" required>
                                                ...
                                                <option value="<?php echo PAYMENT_METHOD_DEBIT_CARD; ?>">Debit Card</option>
                                                <option value="<?php echo PAYMENT_METHOD_CAMPUS_WALLET; ?>">Campus Wallet</option>
                                                <option value="<?php echo PAYMENT_METHOD_COUPONS; ?>">Coupons</option>
                                            </select>
                                        </div>

                                        <div class="form-group">
                                            <label class="form-label" for="special_requests">Special Requests (Optional)</label>
                                            <textarea id="special_requests" name="special_requests" class="form-control" rows="3" placeholder="For example, no onions, extra sauce"></textarea>
                                        </div>

                                        <button type="submit" class="btn btn-primary btn-block btn-lg">
                                            <i class="fas fa-credit-card"></i>
                                            Place Order (R <?php echo number_format($finalTotal, 2); ?>)
                                        </button>
                                    </form>
                                </div>
                            </div>
